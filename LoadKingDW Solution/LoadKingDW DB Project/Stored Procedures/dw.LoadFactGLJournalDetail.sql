--USE [LK-GS-EDW]
--GO
CREATE PROCEDURE dw.sp_LoadFactGLJournalDetail @LoadLogKey INT  AS
...
IF object_id('##FactGLJournalDetail_SOURCE', 'U') is not null -- if table exists
	BEGIN
		Drop table ##FactGLJournalDetail_SOURCE
	END

IF object_id('##FactGLJournalDetail_TARGET', 'U') is not null -- if table exists
	BEGIN
		Drop table ##FactGLJournalDetail_TARGET
	END

	--CREATE TEMP table With SAME structure as destination table (except for IDENTITY field)
	CREATE TABLE ##FactGLJournalDetail_SOURCE (
	[DimDatePostDateSql_Key]     [int] NOT NULL,
	[DimDateTransDateSql_Key]    [int] NOT NULL,
	[DimDateReverseDateSql_Key]     [int] NOT NULL,
	[DimDatePeriodBeginDateSql_Key]  [int] NOT NULL,
	...
	INSERT INTO ##FactGLJournalDetail_SOURCE
			SELECT * from dwstage.V_LoadFactGLJournalDetail

--CREATE TEMP table to be used below for identifying records with CHANGES 

	CREATE TABLE ##FactGLJournalDetail_TARGET 
					(
					      GL_NUMBER [char](15)      NULL
				   ,  POST_DATE_SQL	[date]          NULL
			       ,          BATCH [char](5)       NULL
	...
	INSERT INTO ##FactGLJournalDetail_TARGET
	SELECT	
			        GL_NUMBER 
				   ,POST_DATE_SQL
			       ,BATCH
				   ...
				   ,SEQ
				   ,Type1RecordHash 
	FROM	dw.FactGLJournalDetail

	--INSERT NEW TARGET Items
	INSERT INTO dw.FactGLJournalDetail 
	SELECT	*
	FROM	##FactGLJournalDetail_SOURCE AS SRC
	WHERE	NOT EXISTS(	SELECT  1
						FROM	dw.FactGLJournalDetail AS TGT
						WHERE	
...
,TGT.DimDatePeriodBeginDateSql_Key = SRC.DimDatePeriodBeginDateSql_Key  
...
,TGT.[Type1RecordHash]         = SRC.[Type1RecordHash]  
	...
	FROM	dw.FactGLJournalDetail		AS TGT
	 JOIN   ##FactGLJournalDetail_SOURCE	AS SRC

CREATE PROCEDURE [dw].[sp_LoadLkpJobHeaderHist] @LoadLogKey INT  AS
...
DECLARE @RowsUpdatedCount int
	...
	*/
	...
	--BEGIN TRY DROP TABLE ##LkpJobHeaderHist_SOURCE		END TRY BEGIN CATCH END CATCH
	--BEGIN TRY DROP TABLE ##LkpJobHeaderHist_TARGET	END TRY BEGIN CATCH END CATCH

IF object_id('##LkpJobHeaderHist_SOURCE', 'U') is not null -- if table exists
	BEGIN
		Drop table ##LkpJobHeaderHist_SOURCE
	END

IF object_id('##LkpJobHeaderHist_TARGET', 'U') is not null -- if table exists
	BEGIN
		Drop table ##LkpJobHeaderHist_TARGET
	END
...
DimWorkOrder_Key int NOT NULL,
	...
	[BIN] [char](6) NULL,
	[WO_RTR_RELEASED] [char](1) NULL,--OK
...
----------------------------------------------------
	...
	INSERT INTO ##LkpJobHeaderHist_SOURCE
			SELECT * from dwstage.V_LoadLkpJobHeaderHist

--CREATE TEMP table to be used below for identifying records with CHANGES 

	CREATE TABLE ##LkpJobHeaderHist_TARGET 
					(
					JOB CHAR(6)
					,SUFFIX CHAR(3)
					,PART CHAR(20)
	...
	INSERT INTO ##LkpJobHeaderHist_TARGET
	SELECT	
			JOB
			,SUFFIX
			,PART
			,CUSTOMER
			, Type1RecordHash
	FROM	dw.LkpJobHeaderHist

	--INSERT NEW TARGET Items
	INSERT INTO dw.LkpJobHeaderHist 
	SELECT	*
	FROM	##LkpJobHeaderHist_SOURCE AS SRC
	WHERE	NOT EXISTS(	SELECT  1
						FROM	dw.LkpJobHeaderHist AS TGT
						WHERE	
							TGT.JOB = SRC.JOB
...
, TGT.BIN = SRC.BIN
, TGT.WO_RTR_RELEASED = SRC.WO_RTR_RELEASED
...
, TGT.DATE_DTE_DUE_REV = SRC.DATE_DTE_DUE_REV
...
--------------------------------
, TGT.SCHED_DUE_DATE = SRC.SCHED_DUE_DATE
...
, TGT.OUTS = SRC.OUTS
	...
	FROM	dw.LkpJobHeaderHist		AS TGT
	 JOIN   ##LkpJobHeaderHist_SOURCE	AS SRC
			ON TGT.JOB = SRC.JOB
			and TGT.SUFFIX = SRC.SUFFIX
			and TGT.PART = SRC.PART
			and TGT.CUSTOMER = SRC.CUSTOMER					

CREATE PROCEDURE [dw].[sp_LoadLkpJobOperationsHist] @LoadLogKey INT  AS
...
IF object_id('##LkpJobOperationsHist_SOURCE', 'U') is not null -- if table exists
	BEGIN
		Drop table ##LkpJobOperationsHist_SOURCE
	END

IF object_id('##LkpJobOperationsHist_TARGET', 'U') is not null -- if table exists
	BEGIN
		Drop table ##LkpJobOperationsHist_TARGET
	END
...
[Type1RecordHash]			VARBINARY(64)	NULL	--66 allows for "0x" + 64 characater hash
	...
	INSERT INTO ##LkpJobOperationsHist_SOURCE
			SELECT * from dwstage.V_LoadLkpJobOperationsHist

--CREATE TEMP table to be used below for identifying records with CHANGES 

	CREATE TABLE ##LkpJobOperationsHist_TARGET 
					(
					JOB CHAR(6)
					,SUFFIX CHAR(3)
					,SEQ CHAR(6)
	...
	INSERT INTO ##LkpJobOperationsHist_TARGET
	SELECT	
			JOB
			,SUFFIX
			,SEQ
			...
			,HEADER_CUSTOMER
			, Type1RecordHash
	FROM	dw.LkpJobOperationsHist

	--INSERT NEW TARGET Items
	INSERT INTO dw.LkpJobOperationsHist 
	SELECT	*
	FROM	##LkpJobOperationsHist_SOURCE AS SRC

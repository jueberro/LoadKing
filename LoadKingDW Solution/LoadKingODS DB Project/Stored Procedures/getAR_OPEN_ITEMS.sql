CREATE PROCEDURE [dbo].[getAR_OPEN_ITEMS]
 @SourceTableName varchar(255) --= 'AR_OPEN_ITEMS'
,@LoadLogKey int --= 999
,@StartDate datetime --= '07/07/2020'
,@EndDate datetime --= '07/07/2020'
,@LinkedServer varchar(100) = 'LK-GS-01'
-- EXECUTE [dbo].[getAR_OPEN_ITEMS]
AS
...
SET @SourceTableName = 'AR_OPEN_ITEMS'
SET @LoadLogKey  = 0
...
where 
MasterRunFlag = 'Y' and  tl.TABLE_NAME = 'AR_OPEN_ITEMS' and ec.ExtractEnabledFlag = 1
--and CurRunFlag  <> 'Y' 
...
-- ***** CREATE temp table to do Delta test *********************************************
    ...
    IF object_id('##tmp_AR_OPEN_ITEMS', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_AR_OPEN_ITEMS
	END
    ...
    Set @Sql = 'Select * INTO ##tmp_AR_OPEN_ITEMS From ' +  @BaseSql 

	
    EXEC(@Sql)
    -- select * from ##tmp_AR_OPEN_ITEMS
...
FROM   ( 
      ...
      FROM  ##tmp_AR_OPEN_ITEMS d
	) AS SO
	 
	Drop table ##tmp_AR_OPEN_ITEMS -- select count(*) from ##tmp_JOB_DETAIL

...
CREATE PROCEDURE [dbo].[getPO_RECEIVER]
...
SET @SourceTableName = 'PO_RECEIVER'
SET @LoadLogKey  = 0
...
where 
MasterRunFlag = 'Y' and CurRunFlag  <> 'Y' and  tl.TABLE_NAME = 'PO_RECEIVER' and ec.ExtractEnabledFlag = 1
...
-- ***** CREATE temp table to do Delta test *********************************************
    ...
    IF object_id('##tmp_PO_RECEIVER', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_PO_RECEIVER
	END
    ...
    Set @Sql = 'Select * INTO ##tmp_PO_RECEIVER From ' +  @BaseSql 
	...
	Drop table ##tmp_PO_RECEIVER -- select count(*) from ##tmp_JOB_DETAIL

...
CREATE PROCEDURE dbo.getGL_BALANCES
...
SET @SourceTableName = 'GL_BALANCES'
SET @LoadLogKey  = 0
...
where 
MasterRunFlag = 'Y' and CurRunFlag  <> 'Y' and  tl.TABLE_NAME = 'GL_BALANCES' and ec.ExtractEnabledFlag = 1
...
-- ***** CREATE temp table to do Delta test *********************************************
    ...
    IF object_id('##tmp_GL_BALANCES', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_GL_BALANCES
	END

	IF object_id('##tmp_GL_BALANCE_HISTORY', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_GL_BALANCE_HISTORY
	END

	   -- create the select from source table Openquery using a wildcard
    Set @BaseSql = ' Openquery([' + @LinkedServer  + '],'
    Set @BaseSql = @BaseSql + '''' + 'Select * from  GL_BALANCES '  --Rev4 n.
    Set @BaseSql = @BaseSql + '''' + ' )' 
	  
    Set @Sql = 'Select * INTO ##tmp_GL_BALANCES From '   +  @BaseSql 
    ...
    EXEC(@Sql)
	   ...
	   -- create the select from source table Openquery using a wildcard
    Set @BaseSql = ' Openquery([' + @LinkedServer  + '],'
    Set @BaseSql = @BaseSql + '''' + 'Select * from  GL_BALANCE_HISTORY'  --Rev4 n.
    Set @BaseSql = @BaseSql + '''' + ' )' 
	  
    Set @Sql = 'Select * INTO ##tmp_GL_BALANCE_HISTORY From ' +  @BaseSql 

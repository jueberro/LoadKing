...
CREATE PROCEDURE dbo.getGL_JOURNAL_DTL
...
SET @SourceTableName = 'GL_JOURNAL_DTL'
SET @LoadLogKey  = 0
...
where 
MasterRunFlag = 'Y' and CurRunFlag  <> 'Y' and  tl.TABLE_NAME = 'GL_JOURNAL_DTL' and ec.ExtractEnabledFlag = 1
...
-- ***** CREATE temp table to do Delta test *********************************************
    ...
    IF object_id('##tmp_GL_JOURNAL_DTL', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_GL_JOURNAL_DTL
	END

	IF object_id('##tmp_GL_JOURNAL_DTL_H', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_GL_JOURNAL_DTL_H
	END

	   -- create the select from source table Openquery using a wildcard
    Set @BaseSql = ' Openquery([' + @LinkedServer  + '],'
    Set @BaseSql = @BaseSql + '''' + 'Select * from  GL_JOURNAL_DTL '  --Rev4 n.
    Set @BaseSql = @BaseSql + '''' + ' )' 
	  
    Set @Sql = 'Select * INTO ##tmp_GL_JOURNAL_DTL From '   +  @BaseSql 
    ...
    EXEC(@Sql)
	   ...
	   -- create the select from source table Openquery using a wildcard
    Set @BaseSql = ' Openquery([' + @LinkedServer  + '],'
    Set @BaseSql = @BaseSql + '''' + 'Select * from  GL_JOURNAL_DTL_H'  --Rev4 n.
    Set @BaseSql = @BaseSql + '''' + ' )' 
	  
    Set @Sql = 'Select * INTO ##tmp_GL_JOURNAL_DTL_H From ' +  @BaseSql 
    ...
    INSERT INTO dbo.GL_JOURNAL_DTL

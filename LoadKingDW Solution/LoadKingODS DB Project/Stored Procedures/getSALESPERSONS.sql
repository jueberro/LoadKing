...
SET @SourceTableName = 'SALESPERSONS'
SET @LoadLogKey  = 0
...
where 
MasterRunFlag = 'Y' and CurRunFlag  <> 'Y' and  tl.TABLE_NAME = 'SALESPERSONS' and ec.ExtractEnabledFlag = 1
...
-- ***** CREATE temp table to do Delta test *********************************************
    ...
    IF object_id('##tmp_SALESPERSONS', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_SALESPERSONS
	END
    ...
    Set @Sql = 'Select * INTO ##tmp_SALESPERSONS From ' +  @BaseSql 
	...
	Drop table ##tmp_SALESPERSONS -- select count(*) from ##tmp_JOB_DETAIL

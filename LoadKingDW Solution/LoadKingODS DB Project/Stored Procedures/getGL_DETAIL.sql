CREATE PROCEDURE [dbo].[getGL_DETAIL]
...
where 
MasterRunFlag = 'Y' and CurRunFlag  <> 'Y' and  tl.TABLE_NAME = 'GL_DETAIL' and ec.ExtractEnabledFlag = 1
...
-- ***** CREATE temp table to do Delta test *********************************************
    ...
    IF object_id('##tmp_GL_DETAIL', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_GL_DETAIL
	END

	IF object_id('##tmp_GL_DETAIL_HIST', 'U') is not null -- if table exists
	BEGIN
		Drop table ##tmp_GL_DETAIL_HIST
	END

	   -- create the select from source table Openquery using a wildcard
    Set @BaseSql = ' Openquery([' + @LinkedServer  + '],'
    Set @BaseSql = @BaseSql + '''' + 'Select * from  GL_DETAIL '  --Rev4 n.
    Set @BaseSql = @BaseSql + '''' + ' )' 
	  
    Set @Sql = 'Select * INTO ##tmp_GL_DETAIL From '   +  @BaseSql 
    ...
    EXEC(@Sql)
	   ...
	   -- create the select from source table Openquery using a wildcard
    Set @BaseSql = ' Openquery([' + @LinkedServer  + '],'
    Set @BaseSql = @BaseSql + '''' + 'Select * from  GL_DETAIL_HISTORY'  --Rev4 n.
    Set @BaseSql = @BaseSql + '''' + ' )' 
	  
    Set @Sql = 'Select * INTO ##tmp_GL_DETAIL_HISTORY From ' +  @BaseSql 
  ...
  UNION ALL
	...
	Drop table ##tmp_GL_DETAIL 
    Drop table ##tmp_GL_DETAIL_HISTORY

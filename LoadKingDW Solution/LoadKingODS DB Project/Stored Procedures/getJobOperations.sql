...
--==============================================
--Procedure Name: [LK-GS-ODS].dbo.getJobOperations
--       Created: Pragmatic Works, Edwin Davis 5/12/2020
--       Purpose: Insert a new Batch into ODS File [LK-GS-ODS].ods._V_JobOperations
--==============================================

CREATE PROCEDURE dbo.getJobOperations
...
SELECT 
@SourceTableName = '_V_JobOperations'
,@LoadLogKey = 0
,@StartDate = '1/1/1900'
,@EndDate = getdate() 	
*/
...
EXEC(@Sql)
  ...
  -- create the select from source table Openquery using a wildcard
Set @BaseSql = ' Openquery([LK_GS],'
Set @BaseSql = @BaseSql + '''' + 'Select * from  JOB_OPERATIONS '  --Rev4 n.
Set @BaseSql = @BaseSql + '''' + ' )' 
...
INSERT INTO dbo._V_JobOperations
